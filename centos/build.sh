export PACKER_LOG=1
export PACKER_LOG_PATH=packer_err.log
# packer build 5/centos-5.11-x86_64.json
# packer build 6/centos-6.5-x86_64.json
# packer build 6/centos-6.6-x86_64.json
# packer build 6/centos-6.6-java7_dev-x86_64.json
# packer build 6/centos-6.8-java8_dev-x86_64.json
# packer build 6/centos-6.8-java8-node_dev-x86_64.json
# packer build 6/centos-6.7-x86_64.json
# packer build 6/centos-6.6-java7-x86_64.json
# packer build 7/centos-7.3.json
# packer build 6/centos-6.6-java7-x86_64.json
# packer build 6/centos-6.9-java8-x86_64.json
# packer build 7/centos-7.0.json
# packer build 6/centos-6.9-java8-x86_64.json
# packer build 6/centos-6.9-x86_64.json
# # packer build 7/centos-7.0.json
# packer build 7/centos-7.4.json
# packer build 8/centos-8.0.json
# packer build 8/centos-8.1.json
# packer build 8/centos-8.2.json
# packer build 8/centos-8.3.json
packer build 7/centos-7.9.json
